...
class ProfilerData:
    ...
    def load_buffer(self, thread, core_loc="0,0", num_bytes=BUFFER_SIZE):
        return read_words_from_device(core_loc, self.BUFFER[thread], self.BUFFER_SIZE)

    def parse_raw(self, raw):
        data = {"UNPACK": [], "MATH": [], "PACK": []}
        for thread in data:
            raw_thread = raw[thread]
            data[thread] = self.parse_thread(thread, raw_thread)
        return data
    ...
    def parse_entry(self, word, zone_stack, raw_iter):
